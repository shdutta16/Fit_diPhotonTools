...
class LimitPlot(PlotApp):
    ...
    def __call__(self,options,args):
        ...
        print options.rootstyle_extra, "  this is options.rootstyle_extra"
        ...
        print options.couplings, " this is first print for couplings"
        ...
        for fname in flist:
            ...
            print "coup2 after replacing = ",coup
            ...
            tflist[coup] = tfin
            print type(coup)
            print "length of a tflist = ",len(tflist)
            print "coup = ",coup,"  "," tflist[coup] = ",tflist[coup]
            print " ****** Understood till now ****** "
            print

        self.graphs = []
        for coup,tfile in tflist.iteritems():
            print "end of everything 1"
            if options.do_limits:
                print coup, tfile
                self.plotLimit(options,coup,tfile)
            if options.do_pvalues:
                self.plotPval(options,coup,tfile)
        print "end of everything2"
        self.autosave()
        #print "length of coupling list = ", len(options.couplings)
    ...
    def plotLimit(self,options,coup,tfile):
        print "plotLimit is being called"
        if options.model=="BARY":
        ...
        print "-------------------------------------------------------"
        #scale =[0.14805*500,0.042734*500,0.0086088*500]
        scale =[0.14805*500,0.042734*500,0.0086088*500]
        #scale =[500, 500, 500]
        ...
        observed   = scaleGraph2(observedOrig,scale)   #Debu
        expected68 = scaleGraph2(expected68Orig,scale) #Debu
        expected95 = scaleGraph2(expected95Orig,scale) #Debu
        if options.cleanup:
        ...
        print "check", coup, "  ",[self.scaleByXsec,coup]
        commonStyle = [[self.scaleByXsec,coup]]+basicStyle
        print type(commonStyle)
        ...
        expected95.GetYaxis().SetRangeUser(0.01,50)
        ...
        self.format(canv,options.postproc)
        print "end of plotLimit function"
    ...
    def plotSummary(self,options):
      ...
      print "  Debu couplings length"
      if len(options.couplings) > 0:
        coups = options.couplings
        print coups,"  Debu couplings"
    ...
    def plotComparisons(self,options):
        print "plotComparisons1 function is being called "
        ...
        print "x, coups after plotComparisons", x, coups

    def plotComparison(self,options,coup,observed):
        print "plotComparisons2 function is being called "        
    ...
    def scaleByXsec(self,graph,coup):
        print "scaleByXsec is being called"
    ...
    def loadXsections(self,inmap):
        print "loadXsections function is being called"
        self.xsections_ = {}
