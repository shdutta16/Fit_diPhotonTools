...
bool use_precomputed_quantiles = true, precomputed_median_only = false; 
...
TGraphAsymmErrors *theBand(TFile *file, int doSyst, int whichChannel, BandType type, double width=0.68) {
	...
	cout<<"tree entry = "<<i<<endl;
        ...
        if (use_precomputed_quantiles) {
            ...
            if (type == Median) {
	      cout<<"quantile value = "<<quant<<endl; 
                if (fabs(quant - 0.5) > 0.005 && fabs(quant - (1-width)/2) > 0.005 && fabs(quant - (1+width)/2) > 0.005) {
                    //std::cout << " don't care about " << quant << std::endl;
		  cout<<"rejected quantile value = "<<quant<<endl;
                    continue;
                } else {
                  cout<<"not rejected quantile value = "<<quant<<endl;
		  //std::cout << " will use " << quant << std::endl;
                }
            }
        }
	std::cout << " Mass " << iMass << " Quantile " << quant<< " Limit " << limit <<  " Limit Error " << limitErr <<std::endl;
	std::cout << "====================================================================="<<std::endl;
